...
def getVectors(input_folder, file_names_fn, extension, output_folder, only_words_in_x_entities, words_without_x_entities, cut_first_line=False):
    ...
    for phrase_list in phrase_sets:
        if counter == 0:
            all_phrase_fn = output_folder+"frequency/phrases/" + "class-all-" + str(only_words_in_x_entities)
            phrase_name_fn = output_folder + "names/" + str(only_words_in_x_entities) + ".txt"
        else:
...
dt.write2dArray(convertPPMI( sp.csr_matrix(dt.import2dArray("../data/placetypes/bow/frequency/phrases/class-all"))), "../data/placetypes/bow/ppmi/class-all")
