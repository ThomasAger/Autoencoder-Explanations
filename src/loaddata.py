...
def ngram_corpus(corpus, max_features, ngram_range):
    print('Adding {}-gram features'.format(ngram_range))
    # Create set of unique n-gram from the training set.
    ngram_set = set()
    for input_list in corpus:
    ...
    ngram_corpus = add_ngram(corpus, token_indice, ngram_range)
    print('Average train sequence length: {}'.format(np.mean(list(map(len, corpus)), dtype=int)))
    return ngram_corpus, max_features

def getIMDBSequences(max_features, ngram_range, maxlen):
    print('Loading data...')
