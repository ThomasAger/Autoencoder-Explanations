...
def getVectors(input_folder, file_names_fn, extension, output_folder, only_words_in_x_entities, words_without_x_entities, cut_first_line=False):
    ...
    counter = 0
    all_phrase_fn = [output_folder+"frequency/phrases/" + "class-all-" + str(only_words_in_x_entities), output_folder+"frequency/phrases/class-all"]
    phrase_name_fn = [output_folder + "names/" + str(only_words_in_x_entities) + ".txt", output_folder + "names/all.txt"]
    ...
    for phrase_list in phrase_sets:
        ...
        """
        dt.write1dArray(phrase_list, phrase_name_fn[counter])
        ...
        dt.write2dArray(all_phrases_complete, all_phrase_fn[counter])
...
#getVectors("../data/raw/previous work/placevectors/", "../data/raw/previous work/placeNames.txt", ".photos", "../data/placetypes/bow/", 50, 1, False)
#getVectors("../data/raw/previous work/winevectors/", "../data/raw/previous work/wineNames.txt", "", "../data/wines/bow/", 50, 0, True)
#getVectors("../data/raw/previous work/movieVectors/Tokens/", "../data/raw/previous work/filmIds.txt", ".film", "../data/movies/bow/", 100, 1, True)
...
dt.write2dArray(convertPPMI( sp.csr_matrix(dt.import2dArray("../data/"+data_type+"/bow/frequency/phrases/class-all"))), "../data/"+data_type+"/bow/ppmi/class-all")
...
def writeFromMultiClass(multi_class_fn):
    multi_class = dt.import1dArray(multi_class_fn)
    class_names = []
    class_values = []
    for s in multi_class:
        s = re.split(r'\t+', s)
        class_name = s[0]
        class_value = int(s[1])
