...
def makeTopVectors(filename):
    ...
    vectors = util.io.import2dArray("Rankings/" + filename + ".representation")
    top250names = util.io.import1dArray("filmdata/top250.txt")
    film_names = util.io.import1dArray("filmdata/filmNames.txt")
    ...
    util.io.write2dArray(top_vectors, "Plots/Top174" + filename + ".representation")
    util.io.write1dArray(ordered_names, "Plots/Top174OrderedByOriginalList.txt")

def makeTopVectorsDirections(filename):
    vectors = util.io.import2dArray("Directions/" + filename + "Cut.directions")
    top250names = util.io.import1dArray("filmdata/top250.txt")
    filmnames = util.io.import1dArray("filmdata/filmNames.txt")
    ...
    util.io.write2dArray(top250vectors, "../data/movies/plot/t250" + filename + ".directions")


def plotTopVectors(filename):

    names = util.io.import1dArray("../data/movies/plot/Top174OrderedByOriginalList.txt")
    space = util.io.import2dArray("../data/movies/plot/Top174" + filename + ".representation")
...
def plotClusters(filename):
    names = util.io.import1dArray("Plots/Top174OrderedByOriginalList.txt")
    space = util.io.import2dArray("Plots/Top174" + filename + ".representation")
    cluster_names = util.io.import1dArray("Clusters/films100N0.6H25L3CutLeastSimilarHIGH0.75,0.67.names")
