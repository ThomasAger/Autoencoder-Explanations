...
classification = "all"
tf = None
all_fn = "../data/sentiment/bow/frequency/phrases/class-all-" + str(lowest_amt) + "-" + str(
    highest_amt) + "-" + classification
all_fn_binary = "../data/sentiment/bow/binary/phrases/class-all-" + str(lowest_amt) + "-" + str(
    highest_amt) + "-" + classification
ppmi_fn = "../data/sentiment/bow/ppmi/class-all-"+str(lowest_amt)+"-"+str(highest_amt)+"-" + classification

if os.path.exists(all_fn + ".npz") is False:
    ...
    #tf = dt.import2dArray(all_fn, "i")
    #if dt.fileExists(ppmi_fn) is False:
    tf = sp.csr_matrix(tf)
    sp.save_npz(all_fn, tf)
if tf is None:
    tf = sp.load_npz(all_fn + ".npz")
    tf = sp.csr_matrix(tf)
if os.path.exists(ppmi_fn):
    sparse_ppmi = sp.load_npz(ppmi_fn + ".npz")
else:
    sparse_ppmi = mt.convertPPMISparse( tf)
    sp.save_npz(ppmi_fn, sparse_ppmi)
# Removed as we are using sparse representations now
